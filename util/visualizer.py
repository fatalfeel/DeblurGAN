...
class Visualizer():
    ...
    table td {width: %dpx; height: %dpx; padding: 4px; outline: 4px solid black}
                ...
                if label_html_row != '':
                    label_html += '<tr>%s</tr>' % label_html_row
                # pane col = image row
                self.vis.images(images, nrow=ncols, win=self.display_id + 1, padding=2, opts=dict(title='images'))
                label_html = '<table>%s</table>' % label_html
                self.vis.text(table_css + label_html, win = self.display_id + 2, opts=dict(title='labels'))
            else:
                idx = 1
                for label, image_numpy in visuals.items():
